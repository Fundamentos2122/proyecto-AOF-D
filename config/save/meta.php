...
if (isset($_POST['id'])) {
    ...
    $query = $con->prepare("UPDATE  metas SET titulo=?, descripcion=?, ahorrado=?, fecha=?  WHERE id=? AND user_id=?");
    $resultado = $query->execute(array($titulo, $descripcion, $ahorrado, $fecha, $id, $user_id));
    ...
    $query = $con->prepare("INSERT INTO metas (titulo, descripcion, ahorrado, fecha, user_id, activo) VALUES (:tit, :descr, :aho, :fec, :usr, 1)");
    $resultado = $query->execute(array('tit' => $titulo, 'descr' => $descripcion, 'aho' => $ahorrado, 'fec' => $fecha, 'usr' => $user_id));
